...
export default function ProducerDetailPage() {
  ...
  const [editedName, setEditedName] = useState(producer?.name ?? "");

  const handleSaveName = () => {
    if (editedName.trim() && editedName !== producer?.name) {
      updateProducer.mutate({ name: editedName }, {
  ...
  return (
    <>
      <Link href="/producers" className="mb-4 flex items-center gap-1 text-sm text-zinc-500 hover:text-zinc-800">
        ← Voltar
      </Link>
      ...
      {loadingFarms ? (
        ...
        <div className="mt-6 p-6 text-center text-sm text-zinc-500">
              ...
              <CardHeader className="flex flex-row items-start justify-between">
                <div>
                  <CardTitle className="text-lg">
                    {farm.name}
                    {farm.location && <span className="text-sm font-normal text-muted-foreground ml-2">· {farm.location}</span>}
                  </CardTitle>
                </div>
                <Button
                  size="sm"
                  ...
                  onClick={() => {
                    if (confirm(`Remover acesso à fazenda "${farm.name}"? Esta ação não pode ser desfeita.`)) {
