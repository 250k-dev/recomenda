...
export function QuoteExportDialog({
  ...
  /** Sem lojas respondidas (ou ainda carregando), só o atalho do link aparece. */
  data: QuoteComparison | undefined;
  context: QuotePrintContext;
  /** Abre o link de cotação das lojas (gerar/copiar/enviar). */
  ...
  const allIds = useMemo(() => responses.map((r) => r.id), [responses]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Envie o link para as lojas cotarem, ou exporte a comparação dos preços
            recebidos pelo WhatsApp ou em PDF.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex max-h-[70vh] flex-col gap-5 overflow-y-auto px-6 py-5">
            ...
            <section className="rounded-xl border border-border bg-surface-2 p-4">
              <div className="mb-2 flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-text-strong">
                  Link de cotação para as lojas
                </h3>
              </div>
              <p className="mb-3 text-[13px] text-muted-foreground">
                Gere o link com a condição de pagamento e envie para as lojas
                preencherem os preços.
              </p>
              <Button variant="outline" className="gap-2" onClick={onOpenQuoteLink}>
          ...
          {responses.length === 0 ? (
            <p className="rounded-xl border border-dashed border-border px-4 py-3 text-[13px] text-muted-foreground">
              Nenhuma loja respondeu ainda. Assim que chegarem preços, a comparação
              pode ser exportada por aqui.
            </p>
          ) : (
            <>
              <section className="rounded-xl border border-border bg-surface-2 p-4">
