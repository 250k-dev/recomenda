...
import type { ZapFarmDto, ZapLoadResult } from "@/components/domain/zap/zap-types";
...
async function loadZap(token: string): Promise<ZapLoadResult<ZapFarmDto>> {
  ...
  try {
    ...
    return { ok: true, data: (await response.json()) as ZapFarmDto };
  } catch {
    return {
      ok: false,
      status: 503,
