...
export default function ProducerTimingTemplateDetailPage() {
      ...
      <EmptyState
        ...
        action={
          <Button asChild variant="outline">
            <Link href={routes.produtores.detalhe(producerId)}>Voltar ao produtor</Link>
          </Button>
        }
      />
    );
  ...
  const producerHref = routes.produtores.detalhe(
    producerId,
    onboardingSeason ? { onboarding: "season", farm_id: onbFarmId } : undefined,
  );
    ...
    <div className="animate-fade-in space-y-6">
      <BreadcrumbBack
        items={[
          ...
          {
            label: "Modelos de Recomendação",
            href: `${producerHref}#timing-templates` as Route,
          },
          { label: template.name },
        ]}
      />
